...
from .CustomSceneComponent import CustomSceneComponent as SceneComponent
...
class CustomSessionComponent(CompoundComponent):
    ...
    def set_offsets(self, track_offset, scene_offset):
        ...
        scene_increment = 0
        if self._is_linked():
            CustomSessionComponent._perform_offset_change(track_offset - self._track_offset, scene_offset - self._scene_offset)
        else:
    ...
    def update(self):
        super(CustomSessionComponent, self).update()
    ...
    def _get_minimal_track_offset(self):
        if self._is_linked():
            return CustomSessionComponent._minimal_track_offset
        return self.track_offset()

    def _get_minimal_scene_offset(self):
        if self._is_linked():
            return CustomSessionComponent._minimal_scene_offset
    ...
    def _is_linked(self):
        return self in CustomSessionComponent._linked_session_instances

    def _link(self):
        assert not self._is_linked()
        CustomSessionComponent._linked_session_instances.append(self)

    def _unlink(self):
        assert self._is_linked()
        CustomSessionComponent._linked_session_instances.remove(self)
    ...
    def _perform_offset_change(track_increment, scene_increment):
        ...
        for instance in CustomSessionComponent._linked_session_instances:
        ...
        if not found_negative_offset and instances_covering_session > 0:
            CustomSessionComponent._minimal_track_offset = minimal_track_offset
            CustomSessionComponent._minimal_scene_offset = minimal_scene_offset
            for instance in CustomSessionComponent._linked_session_instances:
    ...
    def set_paste_button(self, button):
        ...
        self._paste_button_slot = self.make_button_slot('paste')
        self._paste_button_slot.subject = button
